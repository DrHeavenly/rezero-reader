...
The single accumulating state document of the Re:Zero Web Novel Overhaul pipeline. See CLAUDE.md §5 for full rules. This file only ever grows or is explicitly superseded — entries are never silently deleted or edited.
...
- Created: 2026-07-23
- Pipeline version: CLAUDE.md v1.0
- Chapters processed: Arc 6, Ch. 1 ("The Dragon Carriage's Way Back"), Ch. 2 ("His Way of Providing a Warm Welcome"), Ch. 3 ("The Little Girl's Cell") — all rendered 2026-07-23. Ch. 1–3 finals normalized to the standing typographic + spelling decisions below (DL-0003, DL-0004).
- Last 25-chapter glossary audit: not yet due (fewer than 25 chapters approved)

---

## Names & Romanization

| Canonical form | Variants seen in source | First chapter seen | Notes |
|---|---|---|---|
| Natsuki Subaru | Subaru | Arc 6 Ch 3 | Protagonist/narrator. |
| Emilia | — | Arc 6 Ch 3 | |
| Julius | Juukulius (not seen spelled out in this chapter's text) | Arc 6 Ch 3 | |
| Anastasia | Anastasia Hoshin (surname not used in this chapter's text) | Arc 6 Ch 3 | Associated name rendered "Eridna" in this chapter's text — ratified as the interim canonical form (DL-0002 does not cover this; see note below). Monitor spelling consistency against future chapters per §5.3/§8.6 before treating as fully settled. |
...
Default policy per CLAUDE.md §5.3: follow WCT's own established Manual of Style. Honorifics are kept where WCT keeps them, dropped where WCT drops them, attached as WCT attaches them. No project-specific honorifics opinion. Any deviation requires a Decision Log entry naming the specific case and reason.
...
| Hikikomori | Hikikomori | romanized Japanese common noun | Capitalized as a loanword | Yes — italicize on first appearance per chapter per §3.6 | Arc 6 Ch 3 |
| Insignia incident | Insignia incident | event name | Title Case ("Insignia") | No | Arc 6 Ch 3 |
| Priestella | Priestella | place (Water Gate City) | Title Case | No | Arc 6 Ch 1 (canonical per DL-0003; source variant "Pristella") |
| Darepanda | Darepanda | invented item/pun name | Capitalized | No | Arc 6 Ch 3 — attested spelling in WCT's own body text (source glosses it as a variant of "Tarepanda" in a deleted translator's note, but the body text itself reads "Darepanda"); ratified as the interim canonical form per §2.1, no external-knowledge substitution. |

---

## Voice Notes

*(All notes below are provisional — one chapter of evidence only. Per §5.5, propose re-review once each character has appeared in 2–3 chapters.)*
...
DL-0001 | 2026-07-23 | Arc 6 Ch. 3 | Third-party "Quickdraft"-style compiling-tool boilerplate (e.g. a placeholder "# My Book" / "## Chapter One" header, and an empty trailing "### Scene One" heading) found wrapping the actual WCT content in a `_raw.md` file is treated as a serialization/tooling artifact under the spirit of §3.10, even though §3.10 does not literally enumerate a third-party compiling tool's template headers. Node 1 removes it and replaces the placeholder heading with the correct `# Chapter N — Title` per §3.4, sourced from WCT's own published chapter title (bibliographic metadata, not invented). | Prevents this judgment from being re-derived chapter by chapter whenever a raw file arrives pre-processed through this (or a similar) compiling tool; keeps the removal auditable and consistent. | scope: global (any future `_raw.md` exhibiting this same compiling-tool boilerplate pattern) | supersedes: —

DL-0002 | 2026-07-23 | Arc 6 Ch. 3 | Some raw source files render all dialogue in script-style cues (`Name: [dialogue]`), a parsing artifact of the sourcing tool rather than WCT's usual inline-quote convention. Node 1 converts brackets to double curly quotes (`Name: [text]` → `Name: "text"`), keeping the "Name:" prefix exactly as positioned in the source, inventing no dialogue tag/verb. Unattributed speakers marked `???:` are left as `???:` — the ambiguity of who is speaking is preserved, not resolved. Sub-case: a wordless collective reaction rendered as a dialogue-style line attributed to a group label (e.g. "Everyone: [——]") containing only a dash and no actual words is given the same mechanical treatment — brackets to quotes, doubled dash to a single em dash — with no new formatting convention invented for it. | Keeps this conversion uniform across any future chapter sourced through the same script-style tooling, and prevents re-litigating the wordless-collective-reaction sub-case each time it recurs. | scope: global (any future `_raw.md` using this `Name: [...]` script format, including its wordless-collective-reaction sub-case) | supersedes: —

DL-0003 | 2026-07-23 | Arc 6 Ch. 1 | Canonical spelling of the Water Gate City is **Priestella**; the WCT source variant **Pristella** is normalized to it in every final. Per the ruling that `REZERO_KNOWLEDGE_BASE.md` / `GLOSSARY.md` decisions carry more authority than any already-finalized chapter (finals may contain mistakes), this supersedes the earlier informal use of "Pristella" in the first-approved Ch. 3 final; that final was re-normalized to "Priestella" on this date. | Locks a single canonical spelling matching the KB so the name never forks across the arc. | scope: global (all chapters, all arcs) | supersedes: —

DL-0004 | 2026-07-23 | Arc 6 Ch. 1 | Typographic house style for all finals is **American literary convention with curly (typographic) quotation marks**: `"…"` for speech, `'…'` for quotes-within-quotes, `'` for apostrophes/elisions (incl. dialect: `'bout`, `fightin'`). Straight ASCII quotes/apostrophes in a final are normalized to their curly forms; line-initial dialogue takes an opening `"`. This supersedes the straight-ASCII-quote rendering used in the first-approved Ch. 3 final, which was re-normalized to curly quotes on this date; Ch. 1–2 were rendered curly from the outset. | KB/CLAUDE.md authority over prior finals; makes the quote convention enforceable and uniform rather than per-chapter drift. | scope: global (all chapters, all arcs) | supersedes: —

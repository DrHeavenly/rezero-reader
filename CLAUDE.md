...
**Version 1.3 · Established 2026-07-23 · Amended 2026-07-23 · This document is the constitution of the pipeline.**

**Amendment Log**

- **v1.3 (2026-07-23):** Named the target *reading experience* explicitly. The project owner clarified the north star: the finished text should read like the official Re:Zero **light novel** (the professionally revised, published edition) — its polish, its pacing, its natural dialogue — **not** merely a cleanly-typeset clone of the WCT web-novel translation. That LN-quality reading experience, for every IF story and the whole main story, is the project's final goal; Style 2 is the vehicle for it. **Critical guardrail — this does NOT weaken §2.1.** The target is the LN's *register and reading experience only, never its content.* Nodes still work solely from WCT's text: they do not consult, import, or reconstruct LN-only scenes, lines, or phrasings; do not retranslate; do not invent (§2.1 and §10.2 stand unchanged). The LN is a *quality bar, not a source.* The one place official/LN usage is already sanctioned is canonical names and terminology, via `REZERO_KNOWLEDGE_BASE.md` and `GLOSSARY.md`. (The LN is copyrighted; no node reproduces LN text, and the specific reference the owner supplied — a Scribd upload of LN Vol. 23 — is bot-gated and cannot be machine-read, so this is encoded from the well-known general character of the published LN vs. the web-novel translation.) Encoded in §1.1 (benchmark) and §10.1 / §10.5 (register target).
...
This project reprocesses those chapters, arc by arc, into text with the page-presence of a professionally published literary novel. The final goal, stated plainly by the project owner: the finished chapters should give the reading experience of the **official Re:Zero light novel** — the professionally revised, published edition — rather than a merely tidied-up version of the web-novel translation. That means refurbishing WCT's language, dialogue, and lines until the page reads with the LN's polish, confident pacing, and natural dialogue. (For pure page/typography, the owner's earlier touchstone was *All the Light We Cannot See*: clean typography, prose that never trips the reader — that still holds.) **This is a quality target, not a licence to change the story:** the LN is the *register* we aim at, never a *source* — the source is always WCT's English (§2.1). No node reads, imports, or reconstructs the LN's content; this remains a formatting-and-polish pass, **not** a retranslation. The output is for personal/project use only, not redistribution.
...
**The reading-experience target (v1.3).** Style 2's north star is the reading experience of the **official Re:Zero light novel** — the professionally revised, published edition — applied to WCT's web-novel *content*. Ask not only "would a novelist have built this passage this way?" but "does this page read the way the published LN reads?": tight, confident prose with the translation stiffness gone, dialogue that sounds authored rather than transcribed, pacing that carries the reader. This is the standard for every IF story and the whole main story. **The guardrail is absolute (§2.1, §10.2): the LN is the register we imitate, never a source we draw from.** No node reads, recalls, or reconstructs LN-specific scenes, lines, phrasings, or restructurings; where the LN and the web novel differ in *content*, the web novel (WCT) wins every time, because that is our only source. The LN informs *how it reads*, never *what it says*. Canonical names/terminology aligned to official usage are the one sanctioned overlap, and come only through `REZERO_KNOWLEDGE_BASE.md` / `GLOSSARY.md`. (The LN is copyrighted; it is a quality bar held in the mind's eye, not a text any node quotes or fetches.)
...
The register we are ultimately aiming at is the **official Re:Zero light novel** — the way its published pages read (see §1.1, §10.1): polished, confident, unstiff, with authored-sounding dialogue. That is the *destination*. For the *technique* that gets prose there — how to actually build a sentence and run a line of dialogue — the working craft benchmark is the English of the great translated realist novels, above all Dostoevsky's *Crime and Punishment* in its standard translations (Garnett; McDuff; Pevear & Volokhonsky). Think of it as: the LN sets the target reading experience; the Dostoevsky-tradition craft below is the toolkit for reaching it. Both are *quality bars to emulate*, never texts to copy or fetch — the LN is copyrighted and neither it nor the owner's supplied Scribd reference (bot-gated, unreadable by machine) is a source; everything below distills well-known craft, always subordinate to §10.2 (nothing added, nothing imported, WCT the sole source of content).
...
- **The attribution balance — the concrete rule.** Across any exchange, mix the four shapes and never default to one: **(a) no tag**, when a clean two-person volley makes the speaker unmistakable; **(b) an action or description beat** that attributes by showing who moves (a gesture, a look, a bit of business) and earns its place by also characterizing; **(c) a plain `said`/`asked`**, the workhorse, its *position* varied — before the line, at a mid-sentence breath, or after; **(d) an evident-tone verb** (`muttered`, `cried`, `put in`), only where §10.2 permits. Two guardrails bound the mix. A **monotony ceiling:** no two adjacent attributed lines should take the same shape, and the mid-sentence `"…," Name said, "…"` split — the Arc 6 Ch 3 failure — is used sparingly, never as the house default. A **clarity floor:** the reader must never have to stop and count backward to work out who is speaking. Drop tags only for as long as a two-person alternation plainly holds; reinsert a name or a beat the instant a third voice enters the scene, roughly three unattributed turns have passed, a beat could be read as another character's, or a run of short lines has blurred the thread (the Gluttony IF failure). The one exception: where the *source itself* leaves a speaker unattributed, preserve that ambiguity deliberately — never resolve it, and never manufacture it by accident. Over-tagging with one shape and under-tagging into a blur are the same defect seen from two sides: attribution that was formula-stamped rather than composed line by line to do a specific job.
...
1. **Dialogue attribution — the primary open problem, and it cuts both ways.** Two opposite failures, one root cause. **Over-tagging:** the Arc 6 Ch 3 overhaul converted `Name: "text"` into good prose but leaned on one shape — `"…," Name said, "…"` — so that, read in bulk, the attribution feels *computed the same way every line* rather than composed; that regularity is itself an AI-tell, no better than the script-cue tell it replaced. **Under-tagging:** the Gluttony IF overhaul, correcting for that, dropped so many tags that in busy scenes the reader loses track of who is speaking — a clarity failure just as damaging. Both come of not composing attribution line by line. The fix is **dialogue-flow rebuilding** to the balance and guardrails now specified in §10.5 ("The attribution balance"): treat a conversation as a unit with its own rhythm; vary verb / position / action-beat / no-tag; hold the monotony ceiling *and* the clarity floor at once. No two adjacent attributions default to the same shape, and no exchange leaves the reader guessing. When a `dialogue-flow` skill or routine is available, Style-2 dialogue passes through it rather than being emitted line-by-line. Until then, this is a manual discipline and a mandatory Review check on Style-2 chapters. **General note: default to polish** — treat every render as wanting a final read purely for craft (rhythm, word choice, and this attribution balance), not only for fidelity.

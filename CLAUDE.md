...
- **Name a speaker's first line.** Dropping the `Name:` cue removes the reader's crutch: when a character speaks for the first time in a scene (or re-enters after being off-page), a tag or beat should mark who it is *within that line*, not only in the narration a paragraph later. If the source reveals the speaker just after, pull that identification up onto the line. Stay light — once a voice is established, let clean volleys run untagged.
